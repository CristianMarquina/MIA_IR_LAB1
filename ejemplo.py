...
class move_straight (smach.State):
	...
	def __init__(self):
		...
		smach.State.__init__(self,outcomes=['success','failure','obstacle_detected'],input_keys=['laser','frame','laser_available'])
	...
	def execute(self,userdata):
		...
		while d>0.75: #This is the control cycle, it keeps moving the robot until something is close (below 0.75 m) 
...
class turn_around (smach.State):
	...
	def __init__(self):
		smach.State.__init__(self,outcomes=['success','failure','free_space_detected'],input_keys=['laser','laser_available'])
	...
	def execute(self,userdata):
		...
		while d<0.75: #This is the control cycle, it keeps turning the robot until there is not obstacle in front 
...
with machine:
	...
	smach.StateMachine.add('MOVE_STRAIGHT',move_straight_state,transitions={'success':'success','failure':'failure','obstacle_detected':'TURN_AROUND'})
	smach.StateMachine.add('TURN_AROUND',turn_around_state,transitions={'success':'success','failure':'failure','free_space_detected':'MOVE_STRAIGHT'})
